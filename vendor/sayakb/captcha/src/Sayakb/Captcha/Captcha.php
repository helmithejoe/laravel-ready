...
class Captcha {
	...
	public static function create($id = null)
	{
		...
		$strhash = static::$config['sensitive'] === true ? static::$char : Str::lower(static::$char);
		$hash = Hash::make($strhash.static::$config['secret']);
		Session::put('captchaHash', $hash);
		...
		*/
		...
		$json = '{"captcha_image":"'.$img.'", "captcha_hash":"'.$hash.'"}';
		echo $json;
	}
	...
	public static function check($value, $hash)
	{
		...
		return $value != NULL AND $captchaHash != NULL AND Hash::check(static::$config['sensitive'] === TRUE ? ($value.static::$config['secret']) : (Str::lower($value).static::$config['secret']), $captchaHash);
	}
